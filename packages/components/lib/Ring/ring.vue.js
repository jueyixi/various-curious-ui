...
const transformColor = require("../packages/utils/transformColor.js");
...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props) {
    vue.ref();
    const props = __props;
    const ringNS = useNS.useNS("ring");
    const { getIndex, setIndex } = useZIndex.useZIndex("ring");
    ...
    vue.onMounted(() => {
      ...
      if (is.isArray(props.color)) {
        colorArr = transformColor.getColor(props.color[0], props.color[1], MAX_VALUE);
      }
    ...
    const drawText = (ctx) => {
      ...
      ctx.fillStyle = fontStyle.color || common.getSkin("--vc-ring-text-color");
    ...
    const drawSubText = (ctx) => {
      ...
      if (props.subTitle) {
        ...
        ctx.fillStyle = subFontStyle.color || common.getSkin("--vc-ring-text-color");
    ...
    return (_ctx, _cache) => {
      return vue.openBlock(), vue.createElementBlock("canvas", {
        class: vue.normalizeClass([vue.unref(ringNS).namespace]),
        id: canvasId.value,
        width: vue.unref(RADIUS) * 2,
        height: vue.unref(RADIUS) * 2
